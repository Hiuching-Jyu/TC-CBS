...
enum conflict_type { DUPLICATE, MUTEX, TARGET, CORRIDOR, RECTANGLE, STANDARD, TYPE_COUNT};

enum conflict_priority { DUPLICATE_CONFIG, CARDINAL, SEMI, NON, UNKNOWN, PRIORITY_COUNT };
...
public:
    ...
    void duplicateConflict(int t1, int t2)
    {
        constraint1.clear();
        constraint2.clear();
        this->constraint1.emplace_back(-1, -1, -1, t1, constraint_type::VERTEX);
        this->constraint2.emplace_back(-1, -1, -1, t2, constraint_type::VERTEX);
        type = conflict_type::DUPLICATE;
        priority = conflict_priority::DUPLICATE_CONFIG;
    }
