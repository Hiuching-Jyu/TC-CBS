...
public:
	...
	bool solve(const Instance& instance, double time_limit, int cost_lowerbound = 0, int cost_upperbound = MAX_COST, bool do_duplicate = 1);
...
private:
	...
	bool generateChild(CBSNode* child, CBSNode* curr, bool do_duplicate);
    size_t generateRoot(bool do_duplicate);

	//conflicts
	void findConflicts(CBSNode& curr, bool do_duplicate);
	void findConflicts(CBSNode& curr, int a1, int a2);

    vector<pair<size_t,size_t>> findDuplicateConflicts(CBSNode& curr, bool do_duplicate);
    void findDuplicateConflicts(bool do_duplicate,CBSNode& curr);
