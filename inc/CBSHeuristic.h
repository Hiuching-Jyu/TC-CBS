...
public:
	...
	bool computeInformedHeuristics(const Instance& instance, CBSNode& curr, double time_limit); // this function is called when poping a CT node for the first time
...
private:
	...
	bool buildWeightedDependencyGraph(const Instance& instance, CBSNode& curr, vector<int>& CG);

	bool dependent(int a1, int a2, CBSNode& node); // return true if the two agents are dependent
	int solve2Agents(const Instance& instance, int a1, int a2, const CBSNode& node, bool cardinal);
