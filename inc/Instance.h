...
public:
    ...
    int getDefaultNumberOfVertex() const{
        std::ifstream mapFile(map_fname);
        if (!mapFile.is_open()) {
            std::cerr << "Failed to open map file: " << map_fname << std::endl;
            return -1;
        }
...
private:
	...
	void saveAgents() const;
      ...
      void generateRandomAgents(int num_of_agents, int warehouse_width);
