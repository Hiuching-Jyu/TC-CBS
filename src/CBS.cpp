                        ...
                        list<shared_ptr<Conflict>>& copy, const list<int>& excluded_agents)
{
    ...
    {
        bool found = false;
//        if(conflict->type == conflict_type::DUPLICATE)
//            continue;
...
//        if (!found && conflict->type != conflict_type::DUPLICATE)
        if (!found )
        {
...
vector<pair<size_t,size_t>> CBS::findDuplicateConflicts(CBSNode& curr, bool do_duplicate)
{
    bool found_duplicate = false;
    vector<pair<size_t, size_t>> result;

    if(do_duplicate == 1 && dupConflict == 1)
    {
        unordered_map<vector<pair<int, int>>, size_t> ref;
        vector<pair<int, int>> cur;
        vector<pair<int, int>> locations;
        vector<int> path_length;
        int max_timestep = curr.makespan + 1;
        for (int i = 0; i < num_of_agents - 1; i++) {
            path_length.push_back((paths[i]->size()));
        }

        sort(path_length.begin(), path_length.end());

        int min_timestep = path_length[0];
        for (size_t timestep_i = 0; timestep_i <= min_timestep; timestep_i++)
        {
            locations.clear();
            for (int a = 0; a < num_of_agents; a++) {
                if (timestep_i >= paths[a]->size())
                    continue;
                else
                    locations.push_back(make_pair(a, paths[a]->at(timestep_i).location));
            }
//        sort(locations.begin(),locations.end());
            ref.insert(pair<vector<pair<int, int>>, size_t>(locations, timestep_i));

            for (size_t timestep_j = timestep_i + 1; timestep_j <= min_timestep; timestep_j++)
            {
                cur.clear();
                for (int a = 0; a < num_of_agents; a++) {
                    if ((timestep_j) >= paths[a]->size())
//                if(!paths[a]->at(timestep_j).location)
                        continue;
                    else
                        cur.push_back(make_pair(a, paths[a]->at(timestep_j).location));
                }
//            sort(cur.begin(), cur.end());
                if (cur == locations) {
                    found_duplicate = true;
                    unordered_map<vector<pair<int, int>>, size_t>::const_iterator iterator = ref.find(cur);
                    if (iterator != ref.end()) {
                        size_t t1 = iterator->second;
                        result.push_back(make_pair(t1, timestep_j));
//                        cout << "t1: " << t1 << "   t2: " << timestep_j << endl;    // for debug
                        break;
                    }
                }
            }
    ...
    if (!found_duplicate || do_duplicate == 0 || dupConflict == 0) {
        result.push_back(make_pair(0,0));
    }
    return result;
}

void CBS::findDuplicateConflicts(bool do_duplicate,CBSNode& curr){
    clock_t t_detect_dupConflict = clock();
    // Detect duplicate conflicts

    vector<pair<size_t, size_t>> if_duplicate;
    if_duplicate = findDuplicateConflicts(curr, do_duplicate);
    if (if_duplicate.front().first != 0 || if_duplicate.front().second != 0)        // if there is duplicate conflict
    {
        size_t t1 = if_duplicate.front().first;
        size_t t2 = if_duplicate.front().second;
//        cout << "duplicate conflict detected" << endl;
        num_duplicate_conflicts += 1;           // calculate duplicate conflicts
//        cout << "t1: " << t1 << "; t2: " << t2 << endl; // for debug
//        printPaths();
        shared_ptr<Conflict> conflict(new Conflict());
        conflict->duplicateConflict(t1, t2);
        assert(!conflict->constraint1.empty());
        assert(!conflict->constraint2.empty());
        curr.conflicts.push_back(conflict);
    }
    runtime_dupConflict += (double) (clock() - t_detect_dupConflict) / CLOCKS_PER_SEC;
}
...
void CBS::findConflicts(CBSNode& curr, bool do_duplicate)
{
    clock_t t = clock();
    if (curr.parent != nullptr)
    {
        findDuplicateConflicts(do_duplicate, curr);
        // Copy from parent
        list<int> new_agents;
...
void CBS::classifyConflicts(CBSNode& node)
{
    ...
    while (!node.unknownConf.empty())
    {
        ...
        if(a == -1)
        {
            con->priority = conflict_priority::DUPLICATE_CONFIG;
        ...
        if (rectangle_helper.strategy != rectangle_strategy::NR &&
            ...
            con->priority != conflict_priority::CARDINAL && con->priority != conflict_priority::DUPLICATE_CONFIG) // not caridnal and duplicate-config vertex conflict
        {
...
bool CBS::generateChild(CBSNode* node, CBSNode* parent, bool do_duplicate)
{
    ...
    if (type == constraint_type::LEQLENGTH) {
            ...
            {
                int a = get<0>(
                        node->constraints.back()); // it is a G-length constraint or a range constraint on this agent
    ...
    findConflicts(*node, do_duplicate);
...
void CBS::printResults() const
{
    ...
    cout << solution_cost << "," << runtime << "," <<
         num_HL_expanded << "," << num_LL_expanded << "," << // HL_num_generated << "," << LL_num_generated << "," <<
          "min_f_val: " << min_f_val << "," << dummy_start->g_val << "," << dummy_start->g_val + dummy_start->h_val << "," <<
         endl;
    cout << "num_of_descendants： " << num_of_descendants << endl;
    cout << "num_of_duplicate_conflicts " << num_duplicate_conflicts << endl;
}
...
void CBS::saveResults(const string& fileName, const string& instanceName) const
{
    ...
    if (!exist)
    {
        ...
        addHeads << "runtime,#high-level expanded,#high-level generated,#low-level expanded,#low-level generated," <<
                 ...
                 "#adopt bypasses," <<
                 "standard conflicts,rectangle conflicts,corridor conflicts,target conflicts,mutex conflicts,duplicate conflicts," <<
                 ...
                 "runtime of rectangle conflicts,runtime of corridor conflicts,runtime of mutex conflicts,runtime of duplicate conflicts," <<
    ...
    stats << runtime << "," <<
          ...
          num_target_conflicts << "," << num_mutex_conflicts << "," << num_duplicate_conflicts << "," <<
          ...
          rectangle_helper.accumulated_runtime << "," << corridor_helper.accumulated_runtime << "," << mutex_helper.accumulated_runtime << "," <<runtime_dupConflict << "," <<
...
bool CBS::solve(const Instance& instance, double _time_limit, int _cost_lowerbound, int _cost_upperbound, bool do_duplicate)
{
    this->min_f_val = _cost_lowerbound;
    this->cost_upperbound = _cost_upperbound;
    this->time_limit = _time_limit;

    num_of_vertex = instance.getDefaultNumberOfVertex();   
    ...
    while (!open_list.empty() && !solution_found)
    {
        ...
        if (!curr->h_computed) // heuristics has not been computed yet
        {
            ...
            runtime = (double) (clock() - start) / CLOCKS_PER_SEC;
            bool succ = heuristic_helper.computeInformedHeuristics(instance, *curr, time_limit - runtime);
            runtime = (double) (clock() - start) / CLOCKS_PER_SEC;
...
//            cout << "makespan: " << makespan  <<  ",  makespan_limit  " << makespan_limit <<  endl;
        ...
        while (foundBypass)
        {
            ...
            if (curr->conflict->type == conflict_type::DUPLICATE)
            {
//                cout << "generating constaints for duplicate conflicts" << endl;
                for(int i = 0 ; i < num_of_child ; i = i + 2)
                {
                    child[i] = new CBSNode();
                    child[i]->constraints = curr->conflict->constraint1;
                    child[i+1] = new CBSNode();
                    child[i+1]->constraints = curr->conflict->constraint2;

                    int a, x, y, t1, t2;
                    constraint_type type;
                    tie(a, x, y, t1, type) = curr->conflict->constraint1.back();
                    tie(a, x, y, t2, type) = curr->conflict->constraint2.back();
                    a = i/2;
                    if(paths[a]->size() > t1 && paths[a]->size() > t2) {

                        x = paths[a]->at(t1).location;
                        child[i]->constraints.emplace_back(a, x, y, t1, constraint_type::VERTEX);
                        child[i]->constraints.pop_front();

                        x = paths[a]->at(t2).location;
                        child[i+1]->constraints.emplace_back(a, x, y, t2, constraint_type::VERTEX);
                        child[i+1]->constraints.pop_front();
                    }
                    else
                    {
                        x = paths[a]->at(paths[a]->size()-1).location;
                        t1 = paths[a]->size()-1;
                        t2 = paths[a]->size()-1;
...
//                        x = paths[a]->at(paths[a]->size()).location;
                        child[i+1]->constraints.emplace_back(a, x, y, t2, constraint_type::VERTEX);
                        child[i+1]->constraints.pop_front();
                    }
                }
            ...
            else if (disjoint_splitting && curr->conflict->type == conflict_type::STANDARD)
            {
                child[0] = new CBSNode();
                child[1] = new CBSNode();
...
//            bool solved[2] = { false, false };
            std::vector<bool> solved(num_of_child, false);
...
//                cout << "num_of_descendants: " << num_of_descendants << endl;
                if (i > 0)
                    paths = copy;
                solved[i] = generateChild(child[i], curr,do_duplicate);
                ...
                if (child[i]->g_val + child[i]->h_val == min_f_val &&
                    curr->unknownConf.size() + curr->conflicts.size() == 0) //no conflicts
                {// found a solution (and finish the while loop)
                    break;
...
//            if(do_duplicate) {
//                runtime = (double) (clock() - start) / CLOCKS_PER_SEC;
//                string filename = "descendantsGen_1.csv";
//                std::ofstream file(filename, std::ios::app);
//                if (file.is_open()) {
//                    if (file.tellp() == 0) {
//                        file << "number of desc,runtime\n";
//                    }
//                    std::ostringstream oss;
//                    oss << num_of_descendants << "," << runtime << "\n";
//                    file << oss.str();
//                    file.close();
//                } else {
...
size_t CBS::generateRoot(bool do_duplicate)
{
    ...
    findConflicts(*dummy_start, do_duplicate);
