...
bool CBSHeuristic::computeInformedHeuristics(const Instance& instance, CBSNode& curr, double _time_limit)
{
	...
	case heuristics_type::WDG:
		if (!buildWeightedDependencyGraph(instance, curr, HG))
			return false;
...
bool CBSHeuristic::buildWeightedDependencyGraph(const Instance& instance, CBSNode& node, vector<int>& CG)
{
	...
	for (const auto& conflict : node.conflicts)
	{
		...
		if (got != lookupTable[a1][a2].end()) // check the lookup table first
		{
			...
			node.conflictGraph[idx] = got->second;
                ...
                save_stats)
		{
			node.conflictGraph[idx] = solve2Agents(instance, a1, a2, node, false);
		...
		{ 
			...
			{
				node.conflictGraph[idx] = solve2Agents(instance, a1, a2, node, cardinal);
...
int CBSHeuristic::solve2Agents(const Instance& instance, int a1, int a2, const CBSNode& node, bool cardinal)
{
    ...
    if (cardinal)
		lowerbound += 1;
    bool do_duplicate = 0;
	cbs.solve(instance, time_limit - runtime, lowerbound, upperbound, do_duplicate);
