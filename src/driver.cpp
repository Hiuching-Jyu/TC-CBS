...
int main(int argc, char** argv)
{
	...
	desc.add_options()
        ...
        ("dupConflict", po::value<bool>()->default_value(true), "detect duplicate conflicts")
		...
		("mutexReasoning", po::value<bool>()->default_value(true), "Using mutex reasoning")
	...
	{
		cbs.clear();
		cbs.solve(instance, vm["cutoffTime"].as<double>(), min_f_val);
		runtime += cbs.runtime;
        num_of_descendants += cbs.num_of_descendants;
	...
	cbs.runtime = runtime;
    cbs.num_of_descendants = num_of_descendants;
