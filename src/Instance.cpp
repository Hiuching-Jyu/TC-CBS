        ...
        map_fname(map_fname), agent_fname(agent_fname), num_of_agents(num_of_agents),  agent_indices(agent_indices)
{
    ...
    if (!succ)
    {
        if (num_of_agents > 0)
        {
            generateRandomAgents(num_of_agents, warehouse_width);
            saveAgents();
        }
        else
        {
...
void Instance::generateRandomAgents(int num_of_agents, int warehouse_width)
{
    ...
    if (warehouse_width == 0)//Generate agents randomly
    {
        ...
        while (k < num_of_agents ) {
            std::default_random_engine e1(std::chrono::system_clock::now().time_since_epoch().count());
            std::uniform_int_distribution<int> u1(0, num_of_rows-2), u2(0, num_of_cols-2);

            int start, x1, y1;
            do {
                x1 = u1(e1);
                y1 = u2(e1);
                start = linearizeCoordinate(x1, y1);
            } while (my_map[start] || starts[start]); // start could not be obstacle, and existed start
            ...
            // find goal
            bool flag = false;
            std::default_random_engine e2(std::chrono::system_clock::now().time_since_epoch().count());
            std::uniform_int_distribution<int> u3(0, num_of_rows-2), u4(0, num_of_cols-2);
            int goal, x2, y2;
            do {
                x2 = u3(e2);
                y2 = u4(e2);
                goal = linearizeCoordinate(x2, y2);
            } while (my_map[goal] || goals[goal] || goal == start); // goal could not be obstacle or existed goal, and current start
